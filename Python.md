...
# запись в result_bool_1 результата логического 'равно'
result_bool_1 = var_1 == var_2    
# result_bool_1 -> False

# запись в result_bool_2 результата логического 'не равно'
result_bool_2 = var_1 != var_2   
# result_bool_2 -> True

# запись в result_bool_3 результата логического 'больше'
result_bool_3 = var_1 > var_2   
# result_bool_3 -> True
```

`if elif else` - конструкция условного оператора

```Python
var_1 = 5

if var_1 < 3:   # если var_1 меньше трех, то
    print('var_1 меньше пяти')
elif var_1 > 7: # иначе если var_1 больше семи, то
    print('var_1 больше семи')
else:           # иначе:
    print('Если ничего выше не истина')
```

`or`, `and` и `not` тоже являются логическими операторами

Оператор `or` используется для выполнения логической операции ИЛИ (OR) над двумя выражениями. Если хотя бы одно из выражений истинно (True), то результат операции будет истиной, иначе результат будет ложью (False).
...
Оператор `and` используется для выполнения логической операции И (AND) над двумя выражениями. Если оба выражения истинны (True), то результат операции будет истиной, иначе результат будет ложью (False).
...
Оператор `not` используется для выполнения логической операции НЕ (NOT) над выражением. Если выражение истинно (True), то результат операции будет ложью (False), и наоборот, если выражение ложно (False), то результат операции будет истиной (True).
...
Обратите внимание, что логические операторы `or` и `and` имеют разный уровень приоритета, поэтому при использовании их вместе в одном выражении следует явно указывать порядок выполнения операций с помощью **скобок**.